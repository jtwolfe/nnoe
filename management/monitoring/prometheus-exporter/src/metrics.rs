use anyhow::Result;
use prometheus::{
    register_gauge_with_registry, Gauge, Opts, Registry,
};
use std::sync::Arc;
...
impl MetricsCollector {
    pub fn new() -> Result<Self> {
        ...
        let agent_uptime = Gauge::with_opts(
            Opts::new("nnoe_agent_uptime_seconds", "Agent uptime in seconds")
        )?;
        registry.register(Box::new(agent_uptime.clone()))?;

        let etcd_connections = Gauge::with_opts(
            Opts::new("nnoe_agent_etcd_connections", "Number of etcd connections")
        )?;
        registry.register(Box::new(etcd_connections.clone()))?;

        let cache_size_bytes = Gauge::with_opts(
            Opts::new("nnoe_agent_cache_size_bytes", "Cache size in bytes")
        )?;
        registry.register(Box::new(cache_size_bytes.clone()))?;

        let cache_size_entries = Gauge::with_opts(
            Opts::new("nnoe_agent_cache_size_entries", "Number of cache entries")
        )?;
        registry.register(Box::new(cache_size_entries.clone()))?;

        let config_updates_total = Gauge::with_opts(
            Opts::new("nnoe_agent_config_updates_total", "Total config updates")
        )?;
        registry.register(Box::new(config_updates_total.clone()))?;

        let service_reloads_total = Gauge::with_opts(
            Opts::new("nnoe_agent_service_reloads_total", "Total service reloads")
        )?;
        registry.register(Box::new(service_reloads_total.clone()))?;

        let dns_query_rate = Gauge::with_opts(
            Opts::new("nnoe_dns_query_rate", "DNS queries per second")
        )?;
        registry.register(Box::new(dns_query_rate.clone()))?;

        let dhcp_lease_count = Gauge::with_opts(
            Opts::new("nnoe_dhcp_lease_count", "Current DHCP lease count")
        )?;
        registry.register(Box::new(dhcp_lease_count.clone()))?;

        let blocked_queries = Gauge::with_opts(
            Opts::new("nnoe_blocked_queries_total", "Total blocked DNS queries")
        )?;
